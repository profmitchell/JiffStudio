...
class ContentViewModel: ObservableObject {
    @Published var importedImages: [NSImage] = []
    ...
    func importPNGSequence() {
        ...
        if panel.runModal() == .OK {
            importedImages = panel.urls.compactMap { NSImage(contentsOf: $0) }
    ...
    func renderPreview() {
        Task {
            do {
                ...
                if let gifData = try? Data(contentsOf: previewURL) {
                    await MainActor.run {
                        self.previewImage = NSImage(data: gifData)
                    }
                }
            } catch {
        ...
        Task {
            do {
                ...
                for time in times {
                    ...
                    await MainActor.run {
                        self.importedImages.append(nsImage)
